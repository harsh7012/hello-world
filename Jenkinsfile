    ...
    environment {
        ...
        CLUSTER_NAME = 'fabulous-creature-1696233915'
        CLUSTER_REGION = 'ap-south-1'
        AWS_CREDENTIALS_ID = 'AWS_CREDENTIALS'
        ARGOCD_CREDENTIALS_ID = 'ARGOCD_CREDENTIALS'
        ARGOCD_SERVER="abc48db7be94c4ca4a9f251ead6c8eeb-1616993000.ap-south-1.elb.amazonaws.com"
        HELM_REPO="chartmuseum"
        HELM_URL="http://a195f9e0e59dc456f909ab9809565124-802003394.ap-south-1.elb.amazonaws.com:8080"
    ...
    stage("Build and Push Helm Charts"){
        ...
        stage('Deploy to cluster') {
            steps {
                ...
                sh 'GIT_SSH_COMMAND="ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i ${GITKEYS}" git clone git@github.com:harsh7012/gitops-deployments.git'
                sh 'sed -i "s/\\(targetRevision:\\) .*/\\1 ${BUILD_TAG_WITHOUT_PR}/" gitops-deployments/${APP_NAME}/application.yaml'
                sh """
                cd gitops-deployments && \
                git config --global user.email "harsh7012@gmail.com" && \
